  ...
  def apply(
    ...
    new Iterator[Row] {
      ...
      def next() = {
        /* IMPLEMENT THIS METHOD */
        if(input.hasNext){
          val currow:Row=input.next()
          val cachekey:Row=cacheKeyProjection(currow)
          val preudfrow:Row=preUdfProjection(currow)
          val posudfrow:Row=postUdfProjection(currow)
          ...
          Row.fromSeq(preudfrow++udfproj++posudfrow)
        }
        else null
      }
    }
