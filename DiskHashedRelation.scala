...
protected [sql] final class GeneralDiskHashedRelation(partitions: Array[DiskPartition])
  ...
  override def getIterator() = {
    /* IMPLEMENT THIS METHOD */
      var PartitionIterator: Iterator[DiskPartition] = partitions.iterator
      PartitionIterator
  }
...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    ...
    if(!inputClosed) {
      ...
      throw new SparkException("No input allowed to closed input.")
    }
  }
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
              ...
              blockSize: Int = 64000) = {
    /* IMPLEMENT THIS METHOD */
      var Diskhashedpartition:Array[DiskPartition] = new Array[DiskPartition](size)
      for(i<- 0 until Diskhashedpartition.size){
        Diskhashedpartition(i)=new DiskPartition("disk" + i.toString(),blockSize)
      }
      while(input.hasNext){
        var onerow:Row=input.next()
        var key:Int=keyGenerator(onerow).hashCode()% size
        Diskhashedpartition(key).insert(onerow)
      }
      for(i <- 0 until Diskhashedpartition.size){
        Diskhashedpartition(i).closeInput()
      }
      val diskhashedp:GeneralDiskHashedRelation = new GeneralDiskHashedRelation(Diskhashedpartition)
      diskhashedp
  }
}
