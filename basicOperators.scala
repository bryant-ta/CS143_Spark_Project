...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    val hashedRelation: DiskHashedRelation = DiskHashedRelation(input, keyGenerator, 3, 64000)
    val partitions:Iterator[DiskPartition]=hashedRelation.getIterator()
    val generator: (Iterator[Row] => Iterator[Row]) = CS143Utils.generateCachingIterator(projectList, child.output)
    var curpartition:Iterator[Row]=null
    ...
    new Iterator[Row] {
      def hasNext() = {
        if(curpartition==null){
          ...
          curpartition.hasNext || fetchNextPartition()
        }
        /* IMPLEMENT THIS METHOD */
      }
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        if(partitions.hasNext){
          curpartition=generator(partitions.next().getData())
          curpartition.hasNext
        }
        else false
      }
    }
